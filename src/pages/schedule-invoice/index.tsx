...
const ScheduleInvoicePage = () => {
  ...
  const fetchInvoices = async (
    ...
    try {
      ...
      const response = await axiosInstance.get(`/schedule-invoice/company/${id}`, {
        params
      });
  ...
  const handleDelete = async (id: string) => {
    try {
      await axiosInstance.patch(`/schedule-invoice/${id}`, { isDeleted: true });
    ...
    try {
      ...
      if (currentInvoice) {
        ...
        response = await axiosInstance.patch(
          `/schedule-invoice/${currentInvoice._id}`,
          payload
        );
  ...
  const handleClose = () => {
   ...
   const handleCreateInvoice = () => {
    navigate(`/admin/company/${id}/schedule-invoice/new`);
  };
    ...
    <div className="mb-2 rounded-md bg-white p-4 shadow-lg">
      <div className="mb-2 flex flex-col justify-between">
        <div className="flex flex-row items-center justify-between">
          <h1 className="mb-2 text-3xl font-bold">Schedule Invoice Management</h1>
 ...
 <Button variant="theme" onClick={() => handleCreateInvoice()}>
              <PlusCircle className="mr-2 h-4 w-4" />
              New Schedule Invoice
            </Button>
          </div>
        </div>

        <div className="mt-4 flex flex-row gap-8 items-center justify-start  ">
          {/* <Input
            type="text"
            value={searchTerm}
...
export default ScheduleInvoicePage;
